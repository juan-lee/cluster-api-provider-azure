...
// FlexScaleSetSpec defines the specification for a flex VM scale set.
type FlexScaleSetSpec struct {
	Name           string
	ResourceGroup  string
	ClusterName    string
	Location       string
...
// ResourceName returns the name of the VM scale set.
func (s *FlexScaleSetSpec) ResourceName() string {
	return s.Name
}

// ResourceGroupName returns the name of the resource group.
func (s *FlexScaleSetSpec) ResourceGroupName() string {
	return s.ResourceGroup
}

// OwnerResourceName is a no-op for VM scale sets.
func (s *FlexScaleSetSpec) OwnerResourceName() string {
	return ""
}

// Parameters returns the parameters for the VM scale set.
func (s *FlexScaleSetSpec) Parameters(existing interface{}) (params interface{}, err error) {
	if existing != nil {
		if _, ok := existing.(compute.VirtualMachineScaleSet); !ok {
			return nil, errors.Errorf("%T is not a compute.VirtualMachineScaleSet", existing)
		}
		// VM scale set already exists
		return nil, nil
	}

	if s.SKU == nil {
		return nil, errors.New("unable to get required scale set SKU from machine cache")
	}

	var faultDomainCount *int32
	faultDomainCountStr, ok := s.SKU.GetCapability(resourceskus.MaximumPlatformFaultDomainCount)
	if !ok {
		return nil, errors.Errorf("unable to get required VM scale set SKU capability %s", resourceskus.MaximumPlatformFaultDomainCount)
	}
	count, err := strconv.ParseInt(faultDomainCountStr, 10, 32)
	if err != nil {
		return nil, errors.Wrapf(err, "unable to parse VM scale set fault domain count")
	}
	faultDomainCount = to.Int32Ptr(int32(count))

	asParams := compute.VirtualMachineScaleSet{
