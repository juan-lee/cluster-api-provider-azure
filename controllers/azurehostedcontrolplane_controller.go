...
// +kubebuilder:rbac:groups=infrastructure.x-k8s.io,resources=azurehostedcontrolplanes,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=infrastructure.x-k8s.io,resources=azurehostedcontrolplanes/status,verbs=get;update;patch
...
func (r *AzureHostedControlPlaneReconciler) reconcileNormal(ctx context.Context, hcpScope *scope.HostedControlPlaneScope, clusterScope *scope.ClusterScope) (reconcile.Result, error) {
	...
	if result, err := hcpService.reconcileControlPlane(req); err != nil {
		return result, err
	}
	...
	switch hcpPod.Status.Phase {
	case corev1.PodRunning:
		hcpScope.Info("HCP pod is running", "instance-id", *hcpScope.GetVMID())
		hcpScope.SetReady()
	case corev1.PodPending:
		hcpScope.Info("HCP pod is pending", "instance-id", *hcpScope.GetVMID())
	case corev1.PodFailed:
	case corev1.PodUnknown:
		hcpScope.SetFailureReason(capierrors.UpdateMachineError)
		hcpScope.SetFailureMessage(errors.New("Pod has gotten into a failued or unknown state"))
	}
